...
contract Crowdsale  {
  using SafeMath for uint;
// maak een copie
  MyToken private token = MyToken(0xEd1C9a256f5F84fA8ec7D29F66a0F1d32B3499dC);
  address  private owner;

  uint private _tokensold;

  event Buy(address buyer, uint tokens);
  // Voor het bijhouden van de beschikbare tokens
  uint private _tokensale;
...
modifier allowstart() {
  require(_tokensale == 0);
  _;
}

modifier allowsale(){
  require (_tokensale > 0);
  require (_tokensale >= msg.value.mul(_convrate).div(Devider)) ;
  _;
}
...
function startCrowdSale () public onlyowner allowstart returns (uint) {
  ...
  _tokensale =  token.allowance(owner, address(this));
}


function BuyTokens () public allowsale payable {

_tokensold = msg.value.mul(_convrate).div(Devider);

_tokensale -= _tokensold;

token.transferFrom(owner, msg.sender, _tokensold);

Buy(msg.sender, _tokensold);
